...
- **작업2 커밋 032c51e (확정 대체 보유 대조)**: 미보유 모델에 확정 대체(fp8 → bf16 promoted)를 안내할 때 그 대체 파일 보유 여부도 대조. reconcileInventory에 altByBase + altHeld(대체 완전일치→원본을 heldSet 포함·이미 있음 dim + "선택: {로더}에서 {보유 파일명}으로 바꿔 주세요", 다운로드 제거)·altVariantCandidates(대체 표기 변형→확정 금지·후보 발화·받기 유지). complete 판정에도 반영. 어제 정규화 후보 매칭을 대체 파일명에 확장.
**검증** — build 0 · smoke · e2e 15/15 · regression. 하네스: 환경UI 16/17(기본 단독·접이·직접선택/폴더대조·logPath 조건부 노출+복사·회귀 / 1 실패는 logPath가 `||prev` 관례로 유지되는 것을 clear로 기대한 테스트 오류), 대체 대조 13/13(altHeld·변형·baseline·렌더 이미있음+선택안내+다운로드 제거). em dash UI 0.
**판단·미결(보고)** — 로그 파일 드래그 수용: 현 드롭존이 `accept=".json"` JSON 전용이라 텍스트 로그 파일 드래그는 드롭존 로직 변경 필요 → 태스크 지시대로 이번엔 경로 안내+복사 행까지만. 드래그 수용은 별도 판단 시 착수. 접이 중첩(env 아코디언 → 다른 방법으로 입력 → 명령어로 확인하는 법)은 입력 폼의 점진적 노출 패턴이라 유지(결과 표시층의 '토글 안 토글 0' 규칙과 구분).
**다음 할 일** — 화면 검수 후 push(사용자 전용). 로그 파일 드래그 수용 판단.
...
- **작업3 커밋 a6b6c88 (자리표시자)**: 3-1 휴리스틱 — 참조 모델명이 한글/CJK·비배포문자 포함 또는 배포 파일명 패턴 실패 시 "제작자가 지정한 자리표시자 파일명으로 보입니다…" 발화(기존 [확인 필요] 행 재사용, 웹검색만 제외). 로더 문맥+동반 모델로 계열 추정 병기(CheckpointLoaderSimple+control_v11p_sd15 → SD1.5 계열 체크포인트). 순수 ASCII 미확인은 현행 유지(웹검색·회귀 가드). 3-2 카탈로그(model_catalog.placeholders, 파일명 완전일치): text_실사모델→alias(확정 대체 Realistic_Vision_V5.1_fp16-no-ema 2.13GB를 promoted로), 스프라이트시트넣는곳·캐릭터이펙트로라넣는곳→user_asset(본인 학습 LoRA, 다운로드 대상 아님). db.note에 책 예제(344p/363p) 근거.
**검증** — build 0 · smoke 3부 · e2e 15/15 · regression. 하네스: 노드 대조 8항(로그 미유입 힌트/유입 이미설치됨·설치대상 분리), 자리표시자 23항(로직 4시나리오 alias/user_asset/휴리스틱+계열/음성가드 + 렌더 RV다운로드·발화·라벨·크래시0). em dash UI 0.
**판단·미결** — 인벤토리(전체 현황 2차 표면)의 웹검색 버튼은 `pit`이 내부 IIFE 스코프라 이번 자리표시자 억제에서 제외(버튼 로직 재구성 필요). 작업 3-1 명시 범위(Solution 확인 필요 행)는 완수. 필요 시 후속으로 인벤토리 억제 추가.
**다음 할 일** — 화면 검수 후 push(사용자 전용). 인벤토리 웹검색 억제 후속 판단.
...
- **수리2 축소판 파일명 선행**: 축소판 드롭존을 `{파일명} · 다른 파일을 올리면 다시 분석합니다`로 재구성. 파일명 선행(C.text 강조·길면 ellipsis), 안내문 후행(dim·nowrap 항상 노출). 클릭·드롭 재분석 무변.
- **IA 중복 병합**: 축소판이 파일명을 보이므로 단계1 헤더 요약에서 파일명 제거, '분석 완료' 상태만 유지(동일 대상 중복 행 금지 준수). 지시 외 최소 조정, 변경요약에 명기.
**검증** — build 0 · smoke 3부 · e2e 15/15. 하네스 8항 전 ✅(파일명 선행·말줄임·dim 후행·헤더 중복 제거·요약 marginLeft 8·마크업 공백 제거·크래시 0). em dash UI 0.
**다음 할 일** — 화면 검수 후 push(사용자 전용).

## 2026-07-14 (입력 존 2단계 재구성 + Solution 헤더 환경 칩)
**한 일** — 커밋 b1f97b6(A), 2b1f5cb(B). A·B 롤백 독립 분리.
- **A 2단계 입력 존**: 드롭존+환경정보를 하나의 라운드박스로 묶고 연번 원형 배지(`NumBadge`)로 단계 표시. 단계1 워크플로우 JSON — 미투입=배지 1 / 분석완료=배지 ✓ + 파일명·'분석 완료' 요약. 드롭존 크기 분기: 미투입=현행(도구 첫 문장), 분석완료=1행 축소판(클릭·드롭으로 다른 파일 재분석). 기존 '다른 파일' 버튼을 축소판 클릭으로 통합. 단계2 내 환경 정보 — 배지 좌·토글 우(NumRow 문법), 미입력=배지 2 amber톤 + 완화 경고 유지 / 입력=배지 ✓ + 요약 강조. 펼침·입력 UI 현행 유지(묶음·배지만 신규). `NumBadge`에 `accent` 옵션 추가(기본 무변, 사용처 1곳 회귀 0). env body 이중 테두리 해제. 스크롤 앵커 `id=env-step`.
- **B Solution 헤더 환경 칩**: 어제의 2b 안내 행('이 진단은 아래 환경 기준입니다…') 삭제 → 같은 정보를 Solution 제목 행 우측·'처방전 저장' 버튼 왼쪽에 칩으로 재배치(카피·레이아웃 일치). 입력=요약 강조(C.point, 좁은 폭에서 경로부터 말줄임) / 미입력=amber '환경 미입력 · 보유 대조 생략됨 · 지금 채우기'. '지금 채우기' → 입력 존 단계2 스크롤 + 환경정보 펼침(`goFillEnv`). 기존 토큰·문법만(evidenceBg 라운드박스 + ANCHOR_LINK). 신규 색·컴포넌트 0.
**검증** — build 0 · smoke 3부 · e2e 15/15. 하네스(A): 단일 박스·배지 1/2→✓·amber톤·축소판 클릭 피커·환경 입력후 접힘 요약. 하네스(B): 2b 삭제·칩 위치(버튼 왼쪽)·미입력 문구·지금 채우기 펼침+스크롤·입력후 요약 전환. 크래시 0 · em dash UI 0.
**판단** — 펼침 중 헤더는 평문 '내 환경 정보'(요약 억제), 접힘 시 요약 노출(어제 동작 유지). 배지 30px·라벨 15는 env 헤더 확립 크기 승계(23 미채택). '지금 채우기'는 in-app 네비라 ANCHOR_LINK 채택.
**다음 할 일** — 화면 검수 후 push(사용자 전용).

## 2026-07-14 (통합 3건: 복사 단위·환경정보 위계·브리핑 눈높이)
**한 일** — 커밋 831a28f.
- **1 복사 단위 = 값 행(CopyRows)**: 공통 컴포넌트 `CopyRows`(여러 줄 값 → 줄 단위 값 행 + 행 우측 복사 아이콘 + 박스 우측 상단 '전체 복사') 신설. '박스+우측상단 복사' 패턴을 CopyRows로 통일 — 방법 A(clone 명령)·AI 정밀 진단 결과 명령 2곳(직전 라운드 방법 A 인라인도 CopyRows로 리팩터). 아이콘 클릭 = 그 줄만 클립보드(개행·안내 미포함). 신규 시각 문법 0. learned JSON·브리핑 텍스트는 통째가 복사 단위라 무변.
- **2 환경정보 위계**: 헤더 '(선택)' 삭제. 접힌 헤더를 상태 표시기로 — 미입력=완화 경고(amber·신규 토큰 0), 입력=환경 요약을 헤더 본문 크기(15)로 강조(부속→주인공). Solution 상단에 동일 요약 재노출("이 진단은 아래 환경 기준입니다: {요약}" / 미입력이면 "환경정보가 없어 보유 대조를 건너뛰었습니다…"). '확인 필요' 모델 행 공통 부속 "LLM 진단받기로 넘기면 출처를 함께 찾아드립니다".
- **3 브리핑 눈높이**: 답변 형식에 조항 추가 — 비개발자 기준 작성·터미널/PowerShell은 GUI 대안(Manager 설치·탐색기 이동·브라우저 다운로드) 없을 때만·명령 시 붙여넣는 위치(실행 위치·프로그램명)까지 안내.
**검증** — build 0 · smoke 3부 · e2e 15/15. 하네스 실측: CopyRows 값 행 클릭=단일 명령(개행 0)·전체=2줄·아이콘 수=명령 수 · 환경 헤더 (선택) 삭제·미입력 경고 · Solution 미입력 요약 · 크래시 0. em dash UI 0.
**판단** — 복사 값 코드·행 부속 폰트는 기존 행 문법(13.5 mono·14 부속)과 동일 유지(본문 15 규칙은 산문 대상). AI 결과 명령은 dev 전용(AI_KEY PROD off)이나 CopyRows 공통화로 함께 적용.
**다음 할 일** — 화면 검수 후 push(사용자 전용).

## 2026-07-13 (실행 블록 복사 단위 개선 · 1안)
**한 일** — 커밋 f824683. 방법 A(직접) clone 코드블록의 복사 단위를 값 행(명령 하나)으로.
- **수리**: `installStep.command`(git clone 다수·줄바꿈 결합)를 줄 단위로 분해 → 각 git clone = 값 행. 행 우측 끝 복사 아이콘(custom_nodes 경로 찾기와 동일 행 문법 재사용·신규 시각 문법 0). 아이콘 클릭 = 그 명령만 클립보드(개행·안내 미포함). 박스 우측 상단은 라벨 '전체 복사'로 명시(전체 유지). 피드백은 기존 Check 스왑 그대로.
- **범위 조사(개별 구현)**: '박스+우측상단 복사' 3곳 — (1)방법 A=수리 (2)AI 정밀 진단 결과(aiResult.command)=dev 전용·비정형 LLM 출력이라 행 분해 부적합·무변 (3)learned JSON 스니펫=통째가 복사 단위·무변. 브리핑은 텍스트 통째 복사(앱 내 코드블록 표시 없음)·무변.
**검증** — build 0 · smoke 3부 · e2e 15/15. 하네스 실측: 값 행 클릭=단일 명령(개행 0)·전체 복사=2줄·아이콘 수=명령 수·크래시 0. em dash UI 0.
**판단** — 값 코드 폰트는 기존 clone 표시(13.5·MONO)와 동일 유지(본문 15 규칙은 산문 대상, 모노 값은 표·캡션 계열). AI 결과 분해는 LLM 출력 파싱 신뢰도 문제로 보류 — 필요 시 별도 지시.
**다음 할 일** — 화면 검수 후 push(사용자 전용).
...
## 2026-07-12 (소형 1건: NumRow 서브카피 노출 규칙)
**한 일** — 커밋 b12b925. Models ② '모델 맞추기 (참조)'의 설명 서브카피("모든 로더 노드의 참조 값 기록…")를 `NumRow` `sub` 슬롯(헤더·상시 노출)에서 펼침 내용(children) 첫 줄 dim 안내로 이동. `NumRow` 컴포넌트에서 `sub` 파라미터·렌더 제거로 규칙 일반화 — 접힘 상태는 배지+제목+토글만(전 행 접힘 높이 균일). `sub` 사용처는 이 1곳뿐이라 전수 정리(grep 0).
**검증** — build 0 · smoke 3부 · e2e 15/15 · 하네스(접힘 서브카피 부재·펼침 첫 줄 노출·크래시 0).
**다음 할 일** — 화면 검수 후 push(사용자 전용).
...
- **2 자세한 진단 펼침 스크롤**: `toggleDetail` 핸들러 — 펼침 시 `#detail-top`(detail 최상단, Summary)으로 스무스 스크롤 1회, 닫을 땐 `if(next)` 가드로 이동 없음. 앵커 div + scrollMarginTop 16.
- **3 Nodes ② 초보자 문장 병합**: '※ 초보자 권장' 별도 줄 삭제 → '노드팩이 일괄 설치됩니다 (초보자 권장 · 반드시 custom_nodes 폴더 안에서 실행).'
- **4 Models 받기 표 버튼-표 간격 +50px**: '모델 한 번에 받기' 버튼 컨테이너 marginBottom 16→66.
- **5 모델 맞추기 서브텍스트 + Mac 스니펫 검증**: '모델 맞추기 (참조)' 행에 sub('모든 로더 노드의 참조 값 기록 · 받기는 위 표에서'). `buildScanSnippet` Mac·Linux는 이미 절대경로 리터럴 정상(find "/Volumes/…" 따옴표, 미입력 시 상대 기본) — 무수리, regression에 Mac 절대/미입력 2케이스 추가 봉인.
**검증** — build 0 · regression(P2.7 스니펫 Mac 2건 신규) · smoke 3부 · e2e 15/15. 스크롤 하네스 실측(펼침 1회·닫힘 0회). em dash 0(가운뎃점·마침표).
**다음 할 일** — 화면 검수 후 push(사용자 전용).
...
**한 일** — 커밋 33af66c.
- **1 인트로 정리**: Node Reference 인트로의 "핵심 파라미터(샘플러·CFG·배치)" dim 줄 삭제(각 노드 슬롯 표에 값 존재·중복). "비활성 노드"를 #3 섹션으로 승격 — 번호 헤더 + 우측 +/- 토글(open.nref3) + 기본 닫힘(1·2번과 동일 문법, 번호는 앞 섹션 렌더 수 기준 동적). 내용 = 노드명(소속 그룹). Node Reference 섹션 게이트에 `inactive.length>0` 추가(비활성만 있어도 렌더). 인트로 문장 줄 0.
- **2 링크 표 헤더**: 제작자 링크 3열 표 "링크" 헤더 텍스트만 열 중앙정렬(textAlign center). 다른 열·행 무변경.
- **발견**: Summary·Node Reference는 "자세한 진단"(detailOpen) 안(기본 닫힘), Solution은 밖(상시). smoke part C에 detailOpen 펼침 + #3 섹션 렌더 실측 추가(mode 4 비활성 노드).
- 검증: build 0 · smoke 3부(표·웹검색·#3 섹션) · e2e 15/15 · 회귀.
**다음 할 일** — 화면 검수 후 push.
