...
import { parseFolderScan, reconcileInventory, buildScanSnippet } from "../src/lib/inventoryMatch.js";
...
// === P2.7 봉인: 대조(inventoryMatch) + 파인딩 m 절대경로 + 결함 d 완비 + 3090 미승격 ===
console.log("\n" + "=".repeat(70) + "\nP2.7: 대조 · 파인딩 m 절대경로 · 완비/깨짐 · 3090 미승격");
{
  let ok = true;
  // (1) 스니펫: 읽기전용(나열)만 — 쓰기·삭제 토큰 0 + 경로 따옴표(공백·한글 방어)
  const winSnip = buildScanSnippet("D:\\ComfyUI\\models", "win");
  const nixSnip = buildScanSnippet("", "unix");
  if (!/Get-ChildItem/.test(winSnip) || /Remove-Item|Del |rmdir|New-Item|Set-Content|Out-File/i.test(winSnip)) { console.log("  ❌ win 스니펫 읽기전용 위반: " + winSnip); fail++; ok = false; }
  if (!/^find "/.test(nixSnip) || /(^|\s)rm\s|rmdir|>\s|(^|\s)mv\s|(^|\s)cp\s/.test(nixSnip)) { console.log("  ❌ unix 스니펫 읽기전용 위반: " + nixSnip); fail++; ok = false; }
  if (!winSnip.includes('"D:\\ComfyUI\\models"')) { console.log("  ❌ win 경로 따옴표 누락"); fail++; ok = false; }
  if (!nixSnip.includes('"ComfyUI/models"')) { console.log("  ❌ unix 기본 경로 누락"); fail++; ok = false; }
  // (2) 파서: PowerShell byte + human GB/MB + 혼용 구분자·대소문자 정규화
  const invPS = parseFolderScan("Checkpoints\\Model_A.safetensors\t23802932552\nvae/model_b.safetensors\t335304388");
  if (invPS.get("model_a.safetensors")?.size !== 23802932552) { console.log("  ❌ PS byte 파싱/대소문자 실패"); fail++; ok = false; }
  if (invPS.get("model_b.safetensors")?.folder !== "vae") { console.log("  ❌ 혼용 구분자(/·\\) 폴더 파싱 실패"); fail++; ok = false; }
  const invHuman = parseFolderScan("model_a.safetensors    23.8 GB\nbroken.safetensors    137 KB");
  if (Math.abs((invHuman.get("model_a.safetensors")?.size || 0) - 23.8e9) > 1e6) { console.log("  ❌ human GB 정규화 실패"); fail++; ok = false; }
  if (invHuman.get("broken.safetensors")?.size !== 137000) { console.log("  ❌ human KB 정규화 실패"); fail++; ok = false; }
  // (3) 완비 판정(결함 d 전제): unknowns 0 + 전부 보유 → complete. 누락/깨짐 → false.
  const synthPlan = { items: [{ selectedFile: "x.safetensors", workflowValue: "checkpoints/x.safetensors", size: "6GB" }], unknowns: [] };
  if (!reconcileInventory(synthPlan, parseFolderScan("checkpoints\\x.safetensors\t6000000000")).complete) { console.log("  ❌ 완비(unknowns 0·보유) complete=false"); fail++; ok = false; }
  const recMiss = reconcileInventory(synthPlan, parseFolderScan("checkpoints\\other.safetensors\t6000000000"));
  if (recMiss.complete || recMiss.results[0].held) { console.log("  ❌ 1개 누락인데 complete/held true"); fail++; ok = false; }
  if (reconcileInventory(synthPlan, parseFolderScan("checkpoints\\x.safetensors\t137216")).complete) { console.log("  ❌ 137KB 깨짐인데 complete=true"); fail++; ok = false; }
  if (reconcileInventory(synthPlan, parseFolderScan("")).scanned) { console.log("  ❌ 미수행(빈)인데 scanned=true"); fail++; ok = false; }
  // (4) 통합: krea2 실 fixture → plan(modelRoot=D:\ComfyUI\models) → 파인딩 m 절대경로 + 3090 미승격
  ...
  if (!chk?.fullPath?.startsWith("D:\\ComfyUI\\models\\diffusion_models")) { console.log(`  ❌ 파인딩m 절대경로 오조립: ${chk?.fullPath}`); fail++; ok = false; }
  if (/models[\\/]models/.test(chk?.fullPath || "")) { console.log("  ❌ 파인딩m 'models' 중복 삽입"); fail++; ok = false; }
  const allHeld = reconcileInventory(plan, parseFolderScan(plan.items.map((it) => `${it.folder}\\${it.selectedFile}\t9999999999`).join("\n"))).results.every((r) => r.held);
  if (!allHeld) { console.log("  ❌ 실 plan 전 파일 나열인데 미보유 잔존"); fail++; ok = false; }
  const main = plan.items.find((i) => i.role === "main_model");
  if (main?.vramWarning || main?.promoted) { console.log(`  ❌ 3090(24GB) 승격 오발동: warn=${!!main?.vramWarning} promoted=${!!main?.promoted}`); fail++; ok = false; }
  if (ok) console.log("  ✅ 스니펫 읽기전용·따옴표 · 파서(byte/GB/KB/혼용) · 완비/누락/깨짐/미수행 · 파인딩m 절대경로(중복없음) · 3090 미승격");
}
