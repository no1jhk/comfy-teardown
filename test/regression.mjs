...
console.log("\n" + "=".repeat(70) + "\nP2.7 최종: 대조 분모 · 스니펫 · 파서발화 · 위치 · 파인딩m · 3090");
{
  ...
  // (파인딩 m 재수리·3090) 입력=models 루트(D:\ComfyModels, "models" 세그먼트 없음): 받기 bat 대상 = 절대 "{입력}\{종류}\{파일}", "\models\" 삽입 0. 미입력 → 상대 폴백.
  const rep = analyze(normalize(JSON.parse(fs.readFileSync(path.join(FIX, "krea2_simple_full_turbo (리얼감을 살리는 워크플로우) 배포.json"), "utf8"))));
  const plan = buildModelPlan(rep, { gpu: "RTX 3090", modelRoot: "D:\\ComfyModels" });
  const isFileUrlM = (u) => /^https?:\/\//.test(u || "") && !/\/tree\//.test(u || "");
  const batItems = plan.items.filter((it) => (it.confidence === "confirmed" || it.confidence === "workflow_author") && isFileUrlM(it.promoted?.downloadUrl || it.downloadUrl));
  if (!batItems.length) { console.log("  ❌ 파인딩m 받기 bat 항목 0(직링크 확정 모델 없음)"); fail++; ok = false; }
  for (const it of batItems) {
    const tgt = downloadTargetFolder("D:\\ComfyModels", it.promoted ? it.promoted.folder : it.folder);
    if (!tgt.startsWith("D:\\ComfyModels\\")) { console.log(`  ❌ 파인딩m bat 대상 절대 아님: ${tgt}`); fail++; ok = false; }
    if (/[\\/]models[\\/]/i.test(tgt)) { console.log(`  ❌ 파인딩m 'models' 세그먼트 삽입: ${tgt}`); fail++; ok = false; }
  }
  if (downloadTargetFolder("", "models/vae") !== "models\\vae") { console.log("  ❌ 파인딩m 미입력 상대 폴백 실패"); fail++; ok = false; }
  ...
  if (ok) console.log("  ✅ p(Boogu 1/3·₩) · n-1 · n-2 · 6-2 · 6-3 · 파인딩m(입력=models 루트→절대·삽입0, 미입력→상대) · 3090");
}
...
// === 1 Summary 정적 추출: 그룹 현황 · 파이프라인 · 핵심 파라미터 (fixture별 기대값) ===
console.log("\n" + "=".repeat(70) + "\n1 Summary: 그룹 현황 · 파이프라인 · 파라미터");
{
  let ok = true;
  const krea = analyze(normalize(JSON.parse(fs.readFileSync(path.join(FIX, "krea2_simple_full_turbo (리얼감을 살리는 워크플로우) 배포.json"), "utf8")))).structSummary;
  if (krea.groups.length !== 3) { console.log(`  ❌ krea2 그룹 3 기대, 실제 ${krea.groups.length}`); fail++; ok = false; }
  const kMain = krea.groups.find((g) => /main/i.test(g.title)), kSeed = krea.groups.find((g) => /seed/i.test(g.title));
  if (!kMain || kMain.bypassed) { console.log(`  ❌ krea2 Main 활성 아님: ${JSON.stringify(kMain)}`); fail++; ok = false; }
  if (!kSeed || !kSeed.bypassed) { console.log(`  ❌ krea2 SeedVR2 bypass 아님: ${JSON.stringify(kSeed)}`); fail++; ok = false; }
  const pid = analyze(normalize(JSON.parse(fs.readFileSync(path.join(FIX, "57_PiD Upscale.json"), "utf8")))).structSummary;
  if (!/이미지 입력/.test(pid.pipeline || "") || !/업스케일/.test(pid.pipeline || "") || !/저장/.test(pid.pipeline || "")) { console.log(`  ❌ PiD 파이프라인(입력→업스케일→저장 기대): ${pid.pipeline}`); fail++; ok = false; }
  const boogu = analyze(normalize(JSON.parse(fs.readFileSync(path.join(FIX, "boogu_synth.json"), "utf8")))).structSummary;
  const kp = boogu.keyParams || {};
  if (kp.resolution !== "1024x1024" || kp.steps !== 4 || kp.sampler !== "lcm") { console.log(`  ❌ Boogu 파라미터(1024x1024·4·lcm 기대): ${JSON.stringify(kp)}`); fail++; ok = false; }
  if (boogu.groups.length !== 0) { console.log(`  ❌ 그룹 없는 워크플로우인데 groups ${boogu.groups.length}`); fail++; ok = false; } // 그룹 없으면 미표기(무회귀)
  if (ok) console.log("  ✅ krea2 3그룹(Main 활성·SeedVR2 bypass) · PiD 파이프라인(입력→업스케일→저장) · Boogu 1024x1024·4·lcm · 그룹없음 미표기");
}

// === 6: 비활성 노드 데이터(structSummary.inactive) — type·group·mode (Bypassed 1열 표·전체현황 2열 요약의 데이터 계약) ===
console.log("\n" + "=".repeat(70) + "\n6: 비활성 노드 데이터(type·group·mode)");
{
  const wf = { last_node_id: 3, groups: [{ title: "Upscale", bounding: [0, 0, 400, 400] }], nodes: [
    { id: 1, type: "BypassNodeA", mode: 4, pos: [50, 50], size: [100, 50], flags: {}, order: 0, properties: {}, widgets_values: [] },
    { id: 2, type: "MuteNodeB", mode: 2, pos: [50, 500], size: [100, 50], flags: {}, order: 1, properties: {}, widgets_values: [] },
    { id: 3, type: "ActiveNode", mode: 0, pos: [50, 250], size: [100, 50], flags: {}, order: 2, properties: {}, widgets_values: [] },
  ] };
  const inact = analyze(normalize(wf), null).structSummary?.inactive || [];
  let ok = true;
  if (inact.length !== 2) { console.log(`  ❌ 비활성 2개 기대, 실제 ${inact.length}`); fail++; ok = false; }
  const a = inact.find((n) => n.type === "BypassNodeA"), b = inact.find((n) => n.type === "MuteNodeB");
  if (a?.mode !== 4) { console.log(`  ❌ BypassNodeA mode 4 기대, 실제 ${a?.mode}`); fail++; ok = false; }
  if (b?.mode !== 2) { console.log(`  ❌ MuteNodeB mode 2 기대, 실제 ${b?.mode}`); fail++; ok = false; }
  if (!a || !("group" in a)) { console.log("  ❌ inactive 항목에 group 필드 없음(날조 금지·미표기 판정용)"); fail++; ok = false; }
  if (a && a.group !== "Upscale") { console.log(`  ❌ BypassNodeA group 'Upscale' 기대(그룹 bbox 내), 실제 '${a.group}'`); fail++; ok = false; }
  if (inact.some((n) => n.type === "ActiveNode")) { console.log("  ❌ 활성 노드(mode 0)가 inactive에 포함됨"); fail++; ok = false; }
...
// === 감사 대응 회귀: 확정 대체(altHeld) 대조 + logPath (커밋 dbd6a2f) — 자가삭제 하네스 금지, durable 커버 ===
console.log("\n" + "=".repeat(70) + "\n감사 대응: altHeld·altCorrupt·altVariant·logPath");
{
  let ok = true;
  ...
  const wfa = (nodes) => ({ last_node_id: 9, last_link_id: 0, version: 0.4, nodes });
  // 전제: z-image fp8(출처 미상) → bf16 promoted
  const zRep = analyze(normalize(wfa([nd(1, "UNETLoader", "z_image_turbo_fp8_e4m3fn.safetensors"), nd(9, "SaveImage", null)])), null);
  const zPlan = buildModelPlan(zRep, { gpu: "", torch: "", cuda: "", modelRoot: "" });
  const zItem = zPlan.items.find((i) => i.selectedFile === "z_image_turbo_fp8_e4m3fn.safetensors");
  if (zItem?.promoted?.filename !== "z_image_turbo_bf16.safetensors") { console.log(`  ❌ 전제(z-image fp8 promoted=bf16) 실패: ${zItem?.promoted?.filename}`); fail++; ok = false; }
  // 1) altHeld 완전일치: heldSet 제외(dim 아님)·noDownloadSet 포함·complete false
  const recAlt = reconcileInventory(zRep.models, parseFolderScan("diffusion_models\\z_image_turbo_bf16.safetensors\t12000000000"), zPlan);
  const bAlt = recAlt.byFile.get("z_image_turbo_fp8_e4m3fn.safetensors");
  if (bAlt?.altHeld !== "z_image_turbo_bf16.safetensors") { console.log(`  ❌ altHeld 미설정: ${bAlt?.altHeld}`); fail++; ok = false; }
  if (recAlt.heldSet.has("z_image_turbo_fp8_e4m3fn.safetensors")) { console.log("  ❌ altHeld가 heldSet에 포함(dim 오분류)"); fail++; ok = false; }
  if (!recAlt.noDownloadSet.has("z_image_turbo_fp8_e4m3fn.safetensors")) { console.log("  ❌ altHeld가 noDownloadSet 미포함(받기 오노출)"); fail++; ok = false; }
  if (recAlt.complete) { console.log("  ❌ altHeld인데 complete=true(재선택 대기 무시 → 실행 준비 완료 오배너)"); fail++; ok = false; }
  // 2) 대체 표기 변형(altVariantCandidates)은 원본 변형과 분리 산출·altHeld 아님
  const recAV = reconcileInventory(zRep.models, parseFolderScan("diffusion_models\\z-image-turbo-bf16.safetensors\t12000000000"), zPlan);
  const bAV = recAV.byFile.get("z_image_turbo_fp8_e4m3fn.safetensors");
  if (bAV?.altHeld || bAV?.altVariantCandidates?.[0] !== "z-image-turbo-bf16.safetensors") { console.log(`  ❌ 대체 표기 변형 분리 실패: altHeld=${bAV?.altHeld} altVariant=${JSON.stringify(bAV?.altVariantCandidates)}`); fail++; ok = false; }
  // 3) 대체 크기 이상(altCorrupt): 자리표시자 alias(RV 2.13GB) 깨진 대체 → altHeld 불성립·받기 유지
  const phRep = analyze(normalize(wfa([nd(1, "CheckpointLoaderSimple", "text_실사모델.safetensors"), nd(9, "SaveImage", null)])), null);
  const phPlan = buildModelPlan(phRep, { gpu: "", torch: "", cuda: "", modelRoot: "" });
  const recCor = reconcileInventory(phRep.models, parseFolderScan("checkpoints\\Realistic_Vision_V5.1_fp16-no-ema.safetensors\t4000"), phPlan);
  const bCor = recCor.byFile.get("text_실사모델.safetensors");
  if (bCor?.altHeld || bCor?.altCorrupt?.parsedSize !== 4000) { console.log(`  ❌ 깨진 대체 altCorrupt 실패: altHeld=${bCor?.altHeld} altCorrupt=${JSON.stringify(bCor?.altCorrupt)}`); fail++; ok = false; }
  if (recCor.noDownloadSet.has("text_실사모델.safetensors")) { console.log("  ❌ 깨진 대체인데 noDownloadSet 포함(받기 억제 오류)"); fail++; ok = false; }
  const recOkSize = reconcileInventory(phRep.models, parseFolderScan("checkpoints\\Realistic_Vision_V5.1_fp16-no-ema.safetensors\t2130000000"), phPlan);
  if (recOkSize.byFile.get("text_실사모델.safetensors")?.altHeld !== "realistic_vision_v5.1_fp16-no-ema.safetensors") { console.log("  ❌ 정상 크기 대체 altHeld 미성립"); fail++; ok = false; }
